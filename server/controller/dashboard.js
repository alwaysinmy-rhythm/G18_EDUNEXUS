...
const notice_board = async (req,res)=>{
    ...
    try {
        const notices = await pool.query(`SELECT al.Lab_ID, l.title, l.Due_Time 
            FROM Attended_Lab AS al JOIN Lab AS l ON al.Lab_ID = l.Lab_ID WHERE SID = $1 
            AND l.Due_Time > NOW() AND l.Due_Time < NOW() + INTERVAL '1 day'`,[Studentid]);
